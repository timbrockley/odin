...
//--------------------------------------------------------------------------------
sqliteBindInt :: proc(self: ^Self, stmt_handle: ^rawptr, iCol: i32, int32: i32) -> i32 {
	//------------------------------------------------------------
	clearErrorMessage(self)
	//------------------------------------------------------------
	if self.db_handle == nil {
	...
	//------------------------------------------------------------
	rc := sqlite3_bind_int(stmt_handle, iCol, int32)
	//------------------------------------------------------------
	if (rc != SQLITE_OK) {
		return returnErrorCode(self, rc, sqliteErrmsg(self))
	}
...
//--------------------------------------------------------------------------------
sqliteBindInt64 :: proc(self: ^Self, stmt_handle: ^rawptr, iCol: i32, int64: i64) -> i32 {
	//------------------------------------------------------------
	clearErrorMessage(self)
	//------------------------------------------------------------
	if self.db_handle == nil {
	...
	//------------------------------------------------------------
	rc := sqlite3_bind_int64(stmt_handle, iCol, int64)
	//------------------------------------------------------------
	if (rc != SQLITE_OK) {
		return returnErrorCode(self, rc, sqliteErrmsg(self))
	}
...
//--------------------------------------------------------------------------------
sqliteBindUInt64 :: proc(self: ^Self, stmt_handle: ^rawptr, iCol: i32, uint64: u64) -> i32 {
	//------------------------------------------------------------
	clearErrorMessage(self)
	//------------------------------------------------------------
	if self.db_handle == nil {
	...
	//------------------------------------------------------------
	rc := sqlite3_bind_int64(stmt_handle, iCol, i64(uint64))
	//------------------------------------------------------------
	if (rc != SQLITE_OK) {
		return returnErrorCode(self, rc, sqliteErrmsg(self))
	}
...
//--------------------------------------------------------------------------------
sqliteColumnUInt64 :: proc(_: ^Self, stmt_handle: ^rawptr, iCol: i32) -> u64 {
	//------------------------------------------------------------
	return u64(sqlite3_column_int64(stmt_handle, iCol))
	//------------------------------------------------------------
}
//--------------------------------------------------------------------------------
...
getSQLiteColumnsTable :: proc(
	...
	if arena_err != nil {
		return nil, returnError(self, SQLITE_NOMEM, "allocation error: arena_ptr", arena_err)
	}
	arena_allocator := virtual.arena_allocator(arena_ptr)
	//------------------------------------------------------------
	table, table_err := new(SQLiteColumnsTable, arena_allocator)
	if table_err != nil {
		return nil, returnError(self, SQLITE_NOMEM, "allocation error: table", table_err)
	}
...
querySQLiteColumns :: proc(
	...
	if make_err != nil {
		return returnError(self, SQLITE_NOMEM, "allocation error: sqlite_columns", make_err)
	}
...
updateRow :: proc(
	...
	//------------------------------------------------------------
	sqlite_columns_map := make(map[string]^SQLiteColumn, allocator = context.temp_allocator)
	//------------------------------------------------------------
	for &sqlite_column in sqlite_columns {
		sqlite_columns_map[string(sqlite_column.name)] = &sqlite_column
	}
	defer delete(sqlite_columns_map)
	//------------------------------------------------------------
	id := typeid_of(T)
	...
	for index := 0; index < count; index += 1 {
		...
		//------------------------------------------------------------
		sqlite_column := sqlite_columns_map[field.name]
		//------------------------------------------------------------
		switch field.type.id {
		case typeid_of(u64):
			err = setSQLiteRowValue(row, field.name, sqlite_column.uint64)
		case typeid_of(i64):
			err = setSQLiteRowValue(row, field.name, sqlite_column.integer)
		case typeid_of(f64):
			err = setSQLiteRowValue(row, field.name, sqlite_column.float)
		case typeid_of(string):
			text_bytes := make([]u8, int(sqlite_column.len), allocator) or_return
			copy(text_bytes, (cast([^]u8)sqlite_column.ptr)[:int(sqlite_column.len)])
			err = setSQLiteRowValue(row, field.name, string(text_bytes))
		case typeid_of([]u8):
			blob := make([]u8, int(sqlite_column.len), allocator) or_return
			copy(blob, (cast([^]u8)sqlite_column.ptr)[:int(sqlite_column.len)])
			err = setSQLiteRowValue(row, field.name, blob)
		case:
...
//--------------------------------------------------------------------------------
updateRowMap :: proc(
	row: ^map[string]SQLiteColumnValue,
	sqlite_columns: []SQLiteColumn,
	allocator: mem.Allocator = context.allocator,
) -> (
	err: Error,
	...
	for sqlite_column in sqlite_columns {
		//------------------------------------------------------------
		key := string(sqlite_column.name)
		//------------------------------------------------------------
		switch sqlite_column.column_type {
		case .SQLITE_UNKNOWN:
			err = .UnknownColumnType

		case .SQLITE_INTEGER:
			row[key] = sqlite_column.integer

		case .SQLITE_FLOAT:
			row[key] = sqlite_column.float
...
//################################################################################
//--------------------------------------------------------------------------------
setSQLiteRowValue :: proc(row: ^$T, field_name: string, value: any) -> Error {
	//----------------------------------------
	struct_field := reflect.struct_field_by_name(typeid_of(T), field_name)
	//----------------------------------------
	if struct_field.type == nil do return .FieldNotFound
	//----------------------------------------
	struct_field_ptr := rawptr(uintptr(row) + struct_field.offset)
	//----------------------------------------
...
foreign sqlite {
	...
	sqlite3_bind_int :: proc "c" (stmt_handle: ^rawptr, iCol: i32, int32: i32) -> i32 ---
	sqlite3_bind_int64 :: proc "c" (stmt_handle: ^rawptr, iCol: i32, int64: i64) -> i32 ---
	...
	sqlite3_prepare_v2 :: proc "c" (db_handle: ^rawptr, sql: cstring, nByte: i32, ppStmt: ^^rawptr, pzTail: ^cstring) -> i32 ---
...
null :: struct {}
//----------------------------------------
// uint64 not currently in SQLiteColumnValue used by sqlite (2026-09-25)
SQLiteColumnValue :: union {
	//----------------------------------------
	null,
	i64,
	f64,
